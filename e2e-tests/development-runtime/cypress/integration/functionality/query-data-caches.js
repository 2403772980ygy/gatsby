 ...
 *    - Regular static page (referred to as `A` in tests, data files, page components, etc)
...
function getExpectedCanonicalPath(config) {
  ...
  return `/query-data-caches/${config.slug}/${
    config.page === `client-only` ? `:client-only` : `page-${config.page}/`
  }`
}
...
function pageTitleAndDataAssertion(config) {
  ...
  cy.getTestElement(`${config.prefix || ``}${config.queryType}-query-result`)
    .invoke(`text`)
    .should(`equal`, `${config.slug}: ${config.data}`)
}
...
describe(`Navigate from page A to page B, invalidate some data resources for page A, navigate back to page A`, () => {
  describe(`Navigating back with gatsby-link`, () => {
    it(`page query`, () => {
      const config = {
        slug: `page-query-A-to-B-to-A-link`,
        queryType: `page`,
        navigateBack: `link`,
        initialPage: `A`,
      }
    ...
    it(`static query`, () => {
      const config = {
        slug: `static-query-A-to-B-to-A-link`,
        queryType: `static`,
        navigateBack: `link`,
        initialPage: `A`,
      }
  ...
  describe(`Navigating back with history.back()`, () => {
    it(`page query`, () => {
      const config = {
        slug: `page-query-A-to-B-to-A-history`,
        queryType: `page`,
    ...
    it(`static query`, () => {
      const config = {
        slug: `static-query-A-to-B-to-A-history`,
        queryType: `static`,
        navigateBack: `history`,
        initialPage: `A`,
      }
